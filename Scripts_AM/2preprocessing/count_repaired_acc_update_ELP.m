...
global CCN;
root='/gpfs51/dors2/gpc/JamesBooth/JBooth-Lab/BDL/AM/ELP/Conn_PhonSem_AM_7_9/raw';
addpath(genpath('/gpfs51/dors2/gpc/JamesBooth/JBooth-Lab/BDL/AM/ELP/typical_data_analysis'));
CCN.func_n='sub*Sem*';
CCN.ses='ses-7';
n=6; %number9of consecutive volumes being replaced. no more than 6 consecutive volumes being repaired.
writefile='accuracy_Sem_behavior_ses-9_test.txt';
subjects = {'sub-5069'}; % if this is empty, it will read data_info.
data_info='/gpfs51/dors2/gpc/JamesBooth/JBooth-Lab/BDL/AM_ELP/Subjects.xlsx';
...
for i=1:length(subjects)
    ...
    for j=1:length(func_f)
        ...
        datafile=[bids_folder '/' subjects{i} '/' CCN.ses '/func/' run_name_e 'events.tsv'];
        ...
        fname=[bids_folder '/' subjects{i} '/' CCN.ses '/func/' run_name '.json'];
        if exist(fname)
        ...
        fprintf(fid,'%s %s %s %d %d %s %.4f %.6f %s %.4f %.6f %s %.4f %.6f %s %.4f %.6f\n', ...
            subjects{i}, run_name, shifted_data_acq, num_repaired, chunks, conditions(1,:), acc_by_condition(1,:), rt_by_condition(1,:), ...
